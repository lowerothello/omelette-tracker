...
void _initTrackData(TrackData *cd, uint16_t songlen)
{
...
static void cb_initTrackData(Event *e)
{
	TrackData *cd = &((TrackChain*)e->dest)->v[(size_t)e->callbackarg].data;
...
void initTrackData(Song *cs, uint8_t index)
{
	TrackChain *newtrack = calloc(1, sizeof(TrackChain) + cs->track->c*sizeof(Track));
	memcpy(newtrack, cs->track, sizeof(TrackChain) + cs->track->c*sizeof(Track));

	_initTrackData(&newtrack->v[index].data, cs->songlen);

	Event e;
	e.sem = M_SEM_SWAP_REQ;
	e.dest = (void **)&cs->track;
	e.src = newtrack;
	e.callback = cb_initTrackData;
...
{
	// initTrackData(cd, cs->songlen); /* TODO: fix lol */
	freeVariantChain(&cd->variant);
...
void _addTrack(Song *cs, uint8_t index)
{
	__addTrack(&cs->track->v[index]);

	cs->track->v[index].data.effect = newEffectChain(cs->track->v[index].output, cs->track->v[index].pluginoutput);

	initTrackData(cs, index);
}
...
{ /* fully atomic */
	...
	if (!count) return; /* no tracks to add */
...
{ /* fully atomic */
	...
	/* TODO: if the last track would be deleted then call
	 *       clearTrackdata(s, &s->track->v[0].data) instead.
	 *       currently MAX(1, index) stops this from ever
	 *       happening */
	count = MIN(count, s->track->c - MAX(1, index));
	if (!count) return; /* no tracks to delete */
...
void deserializeTrack(Song *cs, uint8_t index, FILE *fp, uint8_t major, uint8_t minor)
{
	_addTrack(cs, index);

	Track *cv = &cs->track->v[index];
