...
#define I_MODE_VISUAL 6

#define MAX_INSTRUMENT_INDEX 11
...
void drawMark(uint32_t mark, uint32_t offset)
{
	size_t xpos;
	if (mark >= offset>>1 && mark <= (offset>>1) + w->waveformwidth)
	{
		xpos = (float)(mark - (offset>>1)) / (float)w->waveformwidth * w->waveformw;
		for (size_t i = 0; i < w->waveformh; i++) set_pixel_unsafe(w->waveformcanvas, i%2, xpos, i);
	}
}
void resizeWaveform(instrument *iv)
{
	if (iv->samplelength)
	{
		...
		} else if (w->waveformcursor > iv->length - (w->waveformwidth>>1))
		{
			...
			offset = (iv->length - w->waveformwidth)<<1;
		} else
		{
			cursorxpos = w->waveformw>>1;
			offset = (w->waveformcursor<<1) - (w->waveformwidth);
		}

		if (w->mode == I_MODE_VISUAL)
		{
			if (w->waveformvisual <= offset>>1)
				visualxpos = 0;
			else if (w->waveformvisual >= (offset>>1) + w->waveformwidth)
				visualxpos = w->waveformw;
			else
				visualxpos = (float)(w->waveformvisual - (offset>>1)) / (float)w->waveformwidth * w->waveformw;
		...
		/* draw sample data, TODO: needs to be re-thought, performs badly */
		size_t k, x;
		float channelmix = 1.0 / (float)iv->channels;
		uint32_t maxj = MAX(w->waveformwidth, w->waveformw * w->waveformh * WAVEFORM_OVERSAMPLING);
		double divmaxj = 1.0 / (float)maxj;
		float o = (float)w->waveformh * 0.5;
		float sample;
		for (uint32_t j = 0; j < maxj; j++)
		{
			k = (float)j * divmaxj * (float)w->waveformwidth;
			x = (float)j * divmaxj * (float)w->waveformw;
			...
			if (x < lowxpos || x > highxpos)
				set_pixel_unsafe(w->waveformcanvas, 1, x, sample);
			else
				set_pixel_unsafe(w->waveformcanvas, 0, x, sample);
		}

		/* draw trim/loop */
		drawMark(iv->trim[0], offset);
		drawMark(iv->trim[1], offset);
		drawMark(iv->loop[0], offset);
		drawMark(iv->loop[1], offset);
	}
}
...
void drawInstrument(void)
{
	...
	if (iv)
	{
		...
		switch (w->instrumentindex)
		{
			case 0:
				if (!iv->samplelength)
					printf("\033[%d;%dH", w->centre, INSTRUMENT_INDEX_COLS + (ws.ws_col - INSTRUMENT_INDEX_COLS - 9) / 2);
				else
			...
			case 7:  printf("\033[%d;%dH", y+2, x+35); break;
			case 8:  printf("\033[%d;%dH", y+3, x+35 - w->fieldpointer); break;
			case 9:  printf("\033[%d;%dH", y+2, x+56 - w->fieldpointer); break;
			case 10: printf("\033[%d;%dH", y+3, x+52); break;
			case 11: printf("\033[%d;%dH", y+3, x+56 - w->fieldpointer); break;
		}
...
void instrumentIncFieldPointer(short index)
{
	switch (index)
	{
		...
		case 9: w->fieldpointer--; if (w->fieldpointer < 0) w->fieldpointer = 3; break;
...
void instrumentDecFieldPointer(short index)
{
	switch (index)
	{
		...
		case 9: w->fieldpointer++; if (w->fieldpointer > 3) w->fieldpointer = 0; break;
...
void inputInstrumentHex(unsigned short index, instrument *iv, char value)
{
	switch (index)
	{
		...
		case 5:  break; // a[d,r]
		case 8:  updateFieldPush(&iv->loopramp, value); break;
		case 9:  updateField(w->fieldpointer, (uint32_t *)&iv->cyclelength, value); break;
		case 11: updateFieldPush(&iv->pitchshift, value); break;
...
void instrumentAdjustUp(instrument *iv, short index, char mouse)
{
	...
	if (!mouse)
		switch (index)
		{
			...
			case 8:  if (w->fieldpointer) { if (iv->loopramp < 255 - 16) iv->loopramp+=16; else iv->loopramp = 255; } else if (iv->loopramp < 255) iv->loopramp++; break;
			case 9:  temp = iv->cyclelength; incField(w->fieldpointer, &temp, 0xffff); iv->cyclelength = temp; break;
			case 11: if (w->fieldpointer) { if (iv->pitchshift < 255 - 16) iv->pitchshift+=16; else iv->pitchshift = 255; } else if (iv->pitchshift < 255) iv->pitchshift++; break;
		}
}
void instrumentAdjustDown(instrument *iv, short index, char mouse)
{
	...
	if (!mouse)
		switch (index)
		{
			...
			case 8:  if (w->fieldpointer) { if (iv->loopramp > 16) iv->loopramp-=16; else iv->loopramp = 0; } else if (iv->loopramp) iv->loopramp--; break;
			case 9:  temp = iv->cyclelength; decField(w->fieldpointer, &temp); iv->cyclelength = temp; break;
			case 11: if (w->fieldpointer) { if (iv->pitchshift > 16) iv->pitchshift-=16; else iv->pitchshift = 0; } else if (iv->pitchshift) iv->pitchshift--; break;
		}
}
							...
							switch (w->mode)
							{
								...
								case I_MODE_ADJUST: case I_MODE_MOUSEADJUST:
									if (!w->instrumentindex)
									{
										if (iv)
										{
											uint32_t delta = w->waveformwidth / WAVEFORM_COARSE_SLICES;
												...
												case 'D': /* left  */
													switch (w->mode)
													{
														...
														case I_MODE_ADJUST: case I_MODE_MOUSEADJUST:
															...
															{
																uint32_t delta = w->waveformwidth / WAVEFORM_FINE_SLICES;
																if (delta > w->waveformcursor) w->waveformcursor = 0;
																else                           w->waveformcursor -= delta;
																resizeWaveform(iv);
															} redraw(); break;
							...
							switch (w->mode)
							{
								...
								case I_MODE_VISUAL:
									if (w->instrumentindex)
									{
										...
										w->instrumentindex = 1;
										w->fieldpointer = 0;
									} else if (iv) { w->waveformcursor = 0; resizeWaveform(iv); }
									break;
								case I_MODE_NORMAL: case I_MODE_PREVIEW:
									if (w->instrumentindex)
									{
								...
								switch (w->mode)
								{
									...
									case I_MODE_VISUAL:
										if (w->instrumentindex)
										{
											...
											w->fieldpointer = 0;
										} else if (iv) { w->waveformcursor = iv->length-1; resizeWaveform(iv); }
										break;
									case I_MODE_NORMAL: case I_MODE_PREVIEW:
										if (w->instrumentindex)
										{
										...
										if (y < yo)
										{
											...
											switch (y - yo)
											{
												...
												case 2:
													...
													else if (x - xo < 39) { w->instrumentindex = 7; iv->flags ^= S_FLAG_PPLOOP; }
													else
													{
														w->instrumentindex = 9;
												...
												case 3:
													...
													} else if (x - xo < 23) { w->instrumentindex = 6; } // TODO: envelope
													else if (x - xo < 39)
													{
														w->instrumentindex = 8;
														if (x - xo < 36) w->fieldpointer = 1;
														else             w->fieldpointer = 0;
													} else if (x - xo < 55) { w->instrumentindex = 10; iv->flags ^= S_FLAG_TTEMPO; }
													else
													{
							...
							if (w->chord)
							{
								...
								switch (w->chord)
								{
									case 'z':
										switch (input)
										{
											case 'z': /* reset zoom */ if (iv) { w->waveformwidth = iv->length; resizeWaveform(iv); } break;
											case 'i': /* zoom in    */ if (iv) { w->waveformwidth /= 2; resizeWaveform(iv); } break;
											case 'o': /* zoom out   */ if (iv) { w->waveformwidth = MIN(iv->length, w->waveformwidth*2); resizeWaveform(iv); } break;
										} redraw(); break;
								} w->count = 0;
							} else
								switch (input)
								{
									case 'v':  /* normal  */ w->mode = I_MODE_NORMAL; resizeWaveform(iv); redraw(); break;
									case '\t': /* indices */ w->mode = I_MODE_INDICES; resizeWaveform(iv); redraw(); break;
									case 'z':  /* zoom    */ if (!w->instrumentindex) w->chord = 'z'; redraw(); return;
									case '+': case '=': /* zoom in  */ if (iv) { w->waveformwidth /= 2; resizeWaveform(iv); redraw(); } break;
									case '-':           /* zoom out */ if (iv) { w->waveformwidth = MIN(iv->length, w->waveformwidth*2); resizeWaveform(iv); redraw(); } break;
								}
							break;
					} break;
				case I_MODE_NORMAL:
							...
							if (w->chord)
							{
								...
								switch (w->chord)
								{
									...
									case 'm': /* mark */
										switch (input)
										{
											case 't': /* trim start to cursor */ if (iv) { iv->trim[0] = w->waveformcursor; resizeWaveform(iv); } break;
											case 'T': /* trim end to cursor   */ if (iv) { iv->trim[1] = w->waveformcursor; resizeWaveform(iv); } break;
											case 'l': /* loop start to cursor */ if (iv) { iv->loop[0] = w->waveformcursor; resizeWaveform(iv); } break;
											case 'L': /* loop end to cursor   */ if (iv) { iv->loop[1] = w->waveformcursor; resizeWaveform(iv); } break;
										} redraw(); break;
									case 'j': /* jump */
										switch (input)
										{
											case 't': /* cursor to trim start */ if (iv) { w->waveformcursor = iv->trim[0]; resizeWaveform(iv); } break;
											case 'T': /* cursor to trim end   */ if (iv) { w->waveformcursor = iv->trim[1]; resizeWaveform(iv); } break;
											case 'l': /* cursor to loop start */ if (iv) { w->waveformcursor = iv->loop[0]; resizeWaveform(iv); } break;
											case 'L': /* cursor to loop end   */ if (iv) { w->waveformcursor = iv->loop[1]; resizeWaveform(iv); } break;
										} redraw(); break;
								} w->count = 0;
							} else
							{
								if (w->instrumentindex)
									switch (input)
									{
										case '\n': case '\r':
											switch (w->instrumentindex)
											{
												case 6:  break; // sustain
												case 7:  iv->flags ^= S_FLAG_PPLOOP; break;
												case 10: iv->flags ^= S_FLAG_TTEMPO; break;
										...
										case 1:  /* ^a */
											if (iv)
												switch (w->instrumentindex)
												{
													...
													case 4:  break; // channels
													case 5:  break; // a[d,r]
													case 8:  iv->loopramp++; break;
													case 9:  iv->cyclelength++; break;
													case 11: iv->pitchshift++; break;
												}
											redraw(); break;
										case 24: /* ^x */
											if (iv)
									...
									switch (input)
									{
										case '\n': case '\r':
											...
											w->filebrowserCallback = &sampleLoadCallback;
											redraw(); break;
										case 'v':  /* visual  */ w->mode = I_MODE_VISUAL; w->waveformvisual = w->waveformcursor; redraw(); break;
										case 'z':  /* zoom    */ w->chord = 'z'; redraw(); return;
										case '+': case '=': /* zoom in  */ if (iv) { w->waveformwidth /= 2; resizeWaveform(iv); redraw(); } break;
										case '-':           /* zoom out */ if (iv) { w->waveformwidth = MIN(iv->length, w->waveformwidth*2); resizeWaveform(iv); redraw(); } break;
										case 'm':  /* mark    */ w->chord = 'm'; redraw(); return;
