...
void getSampleLoopRamp(uint32_t ptr, uint32_t q, float lerp, Instrument *iv, short *output)
{
	uint8_t shift = 15 - iv->bitdepth;
	*output += ((iv->sampledata[ptr]>>shift)<<shift)*(1.0f - lerp) + ((iv->sampledata[q]>>shift)<<shift)*lerp;
}

/* clamps within range and loop, returns output samples */
void trimloop(uint32_t ptr, uint8_t localenvelope,
		Channel *cv, Instrument *iv, uint8_t stereochannel, short *output)
{
	if (iv->looplength)
...
void samplerProcess(uint8_t realinst, Channel *cv, uint32_t pointer, uint32_t pitchedpointer, short *l, short *r)
{
	...
	{ /* time stretch */
		if (iv->reversegrains)
		{
			trimloop(calcDecimate(iv, iv->samplerate, ((pointer+cyclelength - pointersnap) * calcrate) - (pointersnap * calcrate*calcpitch*calcshift*calcshiftstereol)), localenvelope, cv, iv, 0, l);
			trimloop(calcDecimate(iv, iv->samplerate, ((pointer+cyclelength - pointersnap) * calcrate) - (pointersnap * calcrate*calcpitch*calcshift*calcshiftstereor)), localenvelope, cv, iv, 1, r);
		} else
		{
			trimloop(calcDecimate(iv, iv->samplerate, ((pointer - pointersnap) * calcrate) + (pointersnap * calcrate*calcpitch*calcshift*calcshiftstereol)), localenvelope, cv, iv, 0, l);
			trimloop(calcDecimate(iv, iv->samplerate, ((pointer - pointersnap) * calcrate) + (pointersnap * calcrate*calcpitch*calcshift*calcshiftstereor)), localenvelope, cv, iv, 1, r);
		}
		...
		if (cv->grainrampindex < cv->grainrampmax)
		{
			if (iv->reversegrains)
			{
				trimloop(calcDecimate(iv, iv->samplerate, ((pointer - pointersnap) * calcrate) - ((cyclelength + cv->grainrampindex) * calcrate*calcpitch*calcshift*calcshiftstereol)), localenvelope, cv, iv, 0, &rl);
				trimloop(calcDecimate(iv, iv->samplerate, ((pointer - pointersnap) * calcrate) - ((cyclelength + cv->grainrampindex) * calcrate*calcpitch*calcshift*calcshiftstereor)), localenvelope, cv, iv, 1, &rr);
			} else
			{
				trimloop(calcDecimate(iv, iv->samplerate, ((pointer - pointersnap - cyclelength) * calcrate) + ((cyclelength + cv->grainrampindex) * calcrate*calcpitch*calcshift*calcshiftstereol)), localenvelope, cv, iv, 0, &rl);
				trimloop(calcDecimate(iv, iv->samplerate, ((pointer - pointersnap - cyclelength) * calcrate) + ((cyclelength + cv->grainrampindex) * calcrate*calcpitch*calcshift*calcshiftstereor)), localenvelope, cv, iv, 1, &rr);
			}
	...
	{ /* pitch shift */
		if (iv->reversegrains)
		{
			trimloop(calcDecimate(iv, iv->samplerate, (pointer+cyclelength - pointersnap - (pointersnap * calcshift*calcshiftstereol)) * calcpitch*calcrate), localenvelope, cv, iv, 0, l);
			trimloop(calcDecimate(iv, iv->samplerate, (pointer+cyclelength - pointersnap - (pointersnap * calcshift*calcshiftstereor)) * calcpitch*calcrate), localenvelope, cv, iv, 1, r);
		} else
		{
			trimloop(calcDecimate(iv, iv->samplerate, (pointer - pointersnap + (pointersnap * calcshift*calcshiftstereol)) * calcpitch*calcrate), localenvelope, cv, iv, 0, l);
			trimloop(calcDecimate(iv, iv->samplerate, (pointer - pointersnap + (pointersnap * calcshift*calcshiftstereor)) * calcpitch*calcrate), localenvelope, cv, iv, 1, r);
		}
		...
		if (cv->grainrampindex < cv->grainrampmax)
		{
			if (iv->reversegrains)
			{
				trimloop(calcDecimate(iv, iv->samplerate, (pointer - pointersnap - ((cyclelength + cv->grainrampindex) * calcshift*calcshiftstereol)) * calcpitch*calcrate), localenvelope, cv, iv, 0, &rl);
				trimloop(calcDecimate(iv, iv->samplerate, (pointer - pointersnap - ((cyclelength + cv->grainrampindex) * calcshift*calcshiftstereor)) * calcpitch*calcrate), localenvelope, cv, iv, 1, &rr);
			} else
			{
				trimloop(calcDecimate(iv, iv->samplerate, (pointer - pointersnap - cyclelength + ((cyclelength + cv->grainrampindex) * calcshift*calcshiftstereol)) * calcpitch*calcrate), localenvelope, cv, iv, 0, &rl);
				trimloop(calcDecimate(iv, iv->samplerate, (pointer - pointersnap - cyclelength + ((cyclelength + cv->grainrampindex) * calcshift*calcshiftstereor)) * calcpitch*calcrate), localenvelope, cv, iv, 1, &rr);
			}
