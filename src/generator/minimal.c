void processMinimal(Sample *sample, uint32_t pointer, uint8_t decimate, int8_t bitdepth, short note, short *l, short *r)
{
	...
	{
		getSample(pointer*calcrate*calcpitch * sample->tracks + 0, decimate, bitdepth, sample, l);
		getSample(pointer*calcrate*calcpitch * sample->tracks + 1, decimate, bitdepth, sample, r);
	}
}
