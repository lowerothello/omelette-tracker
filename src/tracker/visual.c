...
void interpolatePartPattern(int8_t x1, int8_t x2, short y1, short y2, uint8_t c1, uint8_t c2)
{
	...
	FOR_VISUAL_TRACKS(i, cv, xstart, xend) // {
		r1 = getTrackRow(cv, y1, 1);
		r2 = getTrackRow(cv, y2, 1);
		for (j = y1; j <= y2; j++)
		{
			r = getTrackRow(cv, j, 1);
			if (xstart <= 0 && xend >= 0)
...
void cycleUpPartPattern(uint8_t count, int8_t x1, int8_t x2, short y1, short y2, uint8_t c1, uint8_t c2)
{
	...
	FOR_VISUAL_TRACKS(i, cv, xstart, xend) // {
		...
		{
			hold = *getTrackRow(cv, y1, 1);
			r2 =    getTrackRow(cv, y2, 1);
			if (xstart <= 0 && xend >= 0)
			{
				for (j = y1; j < y2; j++)
				{
					r0 = getTrackRow(cv, j+0, 1);
					r1 = getTrackRow(cv, j+1, 1);
					r0->note = r1->note;
				}
				r2->note = hold.note;
			}
...
void cycleDownPartPattern(uint8_t count, int8_t x1, int8_t x2, short y1, short y2, uint8_t c1, uint8_t c2)
{
	...
	FOR_VISUAL_TRACKS(i, cv, xstart, xend) // {
		for (l = 0; l < MAX(1, count); l++)
		{
			hold = *getTrackRow(cv, y2, 1);
			r2 =    getTrackRow(cv, y1, 1);

			if (xstart <= 0 && xend >= 0)
			{
				for (j = y2-1; j >= y1; j--)
				{
					r0 = getTrackRow(cv, j+1, 1);
					r1 = getTrackRow(cv, j+0, 1);
					r0->note = r1->note;
				}
				r2->note = hold.note;
			}
...
void bouncePartPattern(short y1, short y2, uint8_t c1, uint8_t c2)
{
	...
	for (row = 0; row < y1; row++)
		for (chnl = 0; chnl <= c2-c1; chnl++)
			ifMacroCallback(0, &spr, chain->v[chnl], getTrackRow(chain->v[chnl], row, 0), 'B', macroBpm);
	buflen = 0;
	...
	for (row = y1; row <= y2; row++)
	{
		for (chnl = 0; chnl <= c2-c1; chnl++)
		{
			r = getTrackRow(chain->v[chnl], row, 0);
			if (r) processRow(0, &spr, 0, chain->v[chnl], r);
		}
