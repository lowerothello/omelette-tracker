int inputTrackVariantEscape(int input)
{
	TrackData *cd = &s->track->v[w->track].data;
	Row *r = getTrackRow(cd, w->trackerfy);
	bool ret = 0;
	...
	switch (input)
	{
		case 1:  /* alt+^a */
			switch (w->mode)
			{
				case T_MODE_VISUAL:     addPartPattern( MAX(1, w->count)*12, MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack), 1, 1); break;
				case T_MODE_VISUALLINE: addPartPattern( MAX(1, w->count)*12, TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack), 1, 0); break;
				case T_MODE_NORMAL: case T_MODE_INSERT: addPartPattern(MAX(1, w->count)*12, tfxToVfx(w->trackerfx), tfxToVfx(w->trackerfx), w->trackerfy, w->trackerfy, w->track, w->track, 1, 1); break;
						...
						if (w->trackerfx > 1) /* cursor is over a macro column */
						{
							...
							{ /* macroc */
								for (int i = MIN(w->trackerfy, w->visualfy); i <= MAX(w->trackerfy, w->visualfy); i++)
									ret = insertMacroc(&getTrackRow(cd, i)->macro[macro], input, 1);
							} else
							{ /* macrov */
								for (int i = MIN(w->trackerfy, w->visualfy); i <= MAX(w->trackerfy, w->visualfy); i++)
									ret = insertMacrov(&getTrackRow(cd, i)->macro[macro], input);
							} regenGlobalRowc(s); return ret;
						} break;
					case T_MODE_VISUAL:     if (input == '~') { altTildePartPattern(MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); return 1; } break;
					case T_MODE_VISUALLINE: if (input == '~') { altTildePartPattern(TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); return 1; } break;
				}
	} return 0;
}

int inputTrackVariant(int input)
{
	...
	switch (w->mode)
	{
		case T_MODE_NORMAL:
			switch (input)
			{
				case '\n': case '\r': /*  mute  */ toggleTrackMute(w->track); p->redraw = 1; break;
				case 's':             /*  solo  */ toggleTrackSolo(w->track); p->redraw = 1; break;
				...
				case '[': /* chnl left          */ trackLeft (MAX(1, w->count)); p->redraw = 1; break;
				case ']': /* chnl right         */ trackRight(MAX(1, w->count)); p->redraw = 1; break;
				...
				case 'x': case 127: case '\b': /* backspace */
					...
					{
						yankPartPattern(0, 1, w->trackerfy, w->trackerfy, w->track, w->track);
						delPartPattern (0, 1, w->trackerfy, w->trackerfy, w->track, w->track);
					} else
					{
						yankPartPattern(tfxToVfx(w->trackerfx), tfxToVfx(w->trackerfx), w->trackerfy, w->trackerfy, w->track, w->track);
						delPartPattern (tfxToVfx(w->trackerfx), tfxToVfx(w->trackerfx), w->trackerfy, w->trackerfy, w->track, w->track);
					} break;
				...
				default: /* column specific */
					...
					switch (w->trackerfx)
					{
						...
						case 1: /* instrument */
							switch (input)
							{
								case 1:  /* ^a */ addPartPattern( MAX(1, w->count), tfxToVfx(w->trackerfx), tfxToVfx(w->trackerfx), w->trackerfy, w->trackerfy, w->track, w->track, 0, 1);
									if (w->instrumentrecv == INST_REC_LOCK_OK) w->instrument = r->inst;
									break;
								case 24: /* ^x */ addPartPattern(-MAX(1, w->count), tfxToVfx(w->trackerfx), tfxToVfx(w->trackerfx), w->trackerfy, w->trackerfy, w->track, w->track, 0, 1);
									if (w->instrumentrecv == INST_REC_LOCK_OK) w->instrument = r->inst;
									break;
							} break;
						default: /* macro */
		...
		case T_MODE_INSERT:
			r = getTrackRow(cd, w->trackerfy);
			if (input == '\n' || input == '\r') /* mute */ { toggleTrackMute(w->track); p->redraw = 1; }
			else
				switch (w->trackerfx)
				{
					...
					case 0:  /* note       */
						...
						regenGlobalRowc(s);
						previewNote(input, getTrackRow(cd, w->trackerfy)->inst);
		...
		case T_MODE_VISUALREPLACE:
			...
			else
			{
				switch (w->trackerfx)
				{
					...
					case 0: /* note */
						for (i = MIN(w->trackerfy, w->visualfy); i <= MAX(w->trackerfy, w->visualfy); i++)
							insertNote(getTrackRow(cd, i), input);
						/* <preview> */
						previewNote(input, getTrackRow(cd, w->trackerfy)->inst);
						/* </preview> */
						regenGlobalRowc(s); break;
					case 1: /* inst */
						for (i = MIN(w->trackerfy, w->visualfy); i <= MAX(w->trackerfy, w->visualfy); i++)
							insertInst(getTrackRow(cd, i), input);
						regenGlobalRowc(s); break;
					default: /* macro */
						macro = (w->trackerfx - 2)>>1;
						if (!(w->trackerfx&1))
						{ /* macroc */
							for (i = MIN(w->trackerfy, w->visualfy); i <= MAX(w->trackerfy, w->visualfy); i++)
								insertMacroc(&getTrackRow(cd, i)->macro[macro], input, 0);
						} else
						{ /* macrov */
							for (i = MIN(w->trackerfy, w->visualfy); i <= MAX(w->trackerfy, w->visualfy); i++)
								insertMacrov(&getTrackRow(cd, i)->macro[macro], input);
						} regenGlobalRowc(s); break;
				} break;
			} break;
		case T_MODE_VISUAL: case T_MODE_VISUALLINE:
			switch (input)
			{
				...
				case '~': /* vi tilde */
					switch (w->mode)
					{
						case T_MODE_VISUAL:     tildePartPattern(MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
						case T_MODE_VISUALLINE: tildePartPattern(TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
					} break;
				case 'i': /* interpolate */
					switch (w->mode)
					{
						case T_MODE_VISUAL:     interpolatePartPattern(MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
						case T_MODE_VISUALLINE: interpolatePartPattern(TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
					} break;
				case '%': /* random */
					switch (w->mode)
					{
						case T_MODE_VISUAL:     randPartPattern(MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
						case T_MODE_VISUALLINE: randPartPattern(TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
					} break;
				case 1: /* ^a */
					switch (w->mode)
					{
						case T_MODE_VISUAL:     addPartPattern( MAX(1, w->count), MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack), 0, 1); break;
						case T_MODE_VISUALLINE: addPartPattern( MAX(1, w->count), TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack), 0, 0); break;
					} regenGlobalRowc(s); break;
				case 24: /* ^x */
					switch (w->mode)
					{
				...
				case 'b': /* bounce visual line to sample */
					bouncePartPattern(MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy),
							MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack));
					w->trackerfx = 0;
					w->trackerfy = MIN(w->trackerfy, w->visualfy);
					w->track = MIN(w->track, w->visualtrack);
				...
				case 'd': case 'x': case 127: case '\b': /* pattern cut */
					...
					{
						case T_MODE_VISUAL:
							yankPartPattern(MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack));
							delPartPattern (MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack));
							break;
						case T_MODE_VISUALLINE:
							yankPartPattern(TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack));
							delPartPattern (TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack));
							break;
					}
				...
				case 'y': /* pattern copy */
					switch (w->mode)
					{
						case T_MODE_VISUAL:     yankPartPattern(MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
						case T_MODE_VISUALLINE: yankPartPattern(TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
					}
					w->trackerfx = MIN(w->trackerfx, vfxToTfx(w->visualfx));
					w->trackerfy = MIN(w->trackerfy, w->visualfy);
					w->track = MIN(w->track, w->visualtrack);
				...
				case '.': /* vtrig loop */
					switch (w->mode)
					{
						case T_MODE_VISUAL:     loopPartPattern(MIN(tfxToVfx(w->trackerfx), w->visualfx), MAX(tfxToVfx(w->trackerfx), w->visualfx), MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
						case T_MODE_VISUALLINE: loopPartPattern(TRACKERFX_VISUAL_MIN, 2+cd->variant->macroc, MIN(w->trackerfy, w->visualfy), MAX(w->trackerfy, w->visualfy), MIN(w->track, w->visualtrack), MAX(w->track, w->visualtrack)); break;
					}
					w->mode = T_MODE_NORMAL;
					p->redraw = 1; break;
				case ';': /* loop range */
