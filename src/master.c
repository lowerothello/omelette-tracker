...
void masterUpArrow   (void *count) { decControlCursor(&cc, (size_t)count * MAX(1, w->count)); p->redraw = 1; }
void masterDownArrow (void *count) { incControlCursor(&cc, (size_t)count * MAX(1, w->count)); p->redraw = 1; }
void masterLeftArrow (void *count) { incControlFieldpointer(&cc); p->redraw = 1; }
void masterRightArrow(void *count) { decControlFieldpointer(&cc); p->redraw = 1; }
void masterHome      (void *count) { setControlCursor(&cc, 0); p->redraw = 1; }
void masterEnd       (void *count) { setControlCursor(&cc, cc.controlc-1); p->redraw = 1; }

void masterPgUp(void *count)
{
	switch (w->page)
	{
